...
sub applyANOVA {
	...
	library("MASS"); # for anova.negbin and glm.nb
	library("pheatmap"); # for pheatmap

	anova.matrix <- function(x, cov) {
		# Check to make sure values are not all 0
		this.result <- matrix(NA, nrow = 1, ncol = 5);
		#colnames(this.result) <- c("df", "deviance", "resid.df", "resid.deviance", "p.value");
		#~ if (length(setdiff(x, rep.int(c(0), length(x)))) != 0) {
		if (sum(x) != 0) {
			this.glm.nb <- glm.nb(x ~ cov);
			this.anova <- anova(this.glm.nb);
			this.result[1,1] <- this.anova$`Df`[2];
			this.result[1,2] <- this.anova$`Deviance`[2];
			this.result[1,3] <- this.anova$`Resid. Df`[2];
			this.result[1,4] <- this.anova$`Resid. Dev`[2];
			this.result[1,5] <- this.anova$`P(>|Chi|)`[2];
		}
		return(this.result);
	}');
	...
	colnames(anova) <- c("df", "deviance", "resid.df", "resid.deviance", "p.value");
